...
namespace LittleQuant.Exchanges.Kingstar
{
    ...
    public class KingstarOptionExchange : IOptionExchange
    {
        private KSFTTraderAdapter _trade;
        private KSFTMDAdapter _market;
        ...
        private OptionAccount _optionAccont = new OptionAccount();
        ...
        public void Initialize()
        {
            ...
            var ret = this._trade.ReqQryInstrument(new ThostFtdcQryInstrumentField { ExchangeID = "SSE" }, this._session.NextRequestID());
            this._asyncWaiter.WaitOne();
        }
        ...
        void _trade_OnRspQryInstrument(ThostFtdcInstrumentField pInstrument, ThostFtdcRspInfoField pRspInfo, int nRequestID, bool bIsLast)
        {
            ...
            if (bIsLast)
            {
                this._market.SubscribeMarketData(this.Instruments.Select(_ => _.ID).ToArray());
            }
        }
        ...
        {
            this._orders.Clear();
            var ret = this._trade.ReqQryOrder(new ThostFtdcQryOrderField { InvestorID = this._config.Account }, this._session.NextRequestID());
        }

        private void QueryAccount()
        {
            var ret = this._trade.ReqQryTradingAccount(new ThostFtdcQryTradingAccountField { BrokerID = this._config.BrokerId, InvestorID = this._config.Account, CurrencyID = "" }, this._session.NextRequestID());
        }
        ...
        private void QueryPositions()
        {
            ...
            var ret = this._trade.ReqQryInvestorPosition(new ThostFtdcQryInvestorPositionField { BrokerID = this._config.BrokerId, InvestorID = this._config.Account }, requestId);
        }

        public OptionAccount Account
        {
            get { return this._optionAccont; }
        }
