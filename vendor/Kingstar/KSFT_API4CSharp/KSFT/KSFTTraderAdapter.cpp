...
KSFTTraderAdapter::KSFTTraderAdapter(String^ pszFlowPath)
{
...
///注册条件单实例
///</summary>
KSFTCosAdapter^ KSFTTraderAdapter::LoadExtApi(System::String^ ExtApiName, KS_EXTAPI_TYPE type)
{
	m_pCosApi = gcnew KSFTCosAdapter;

    m_pCosApi->m_pApi = (CKSCosApi*)m_pApi->LoadExtApi(m_pCosApi->m_pSpi, type);
